...
```
Note that in addition to replacing `if` with `switch`, we have a bunch of type-related stuff in the function definition. While it looks confusing, it's actually helpful because it tells us explicitly the type signatures of our monadic operations:
...
**Problem 2.5.0:** We covered `ret` and `bind`, but what about the other operations we've used: `run`, `raise`, and `tryWith`? Write out their type signatures, and change the old JavaScript code to new TypeScript code.

- Hint 1: `run` is sort of a mirror image of `ret`. How does that impact the type signature?

- Hint 2: `tryWith` is sort of a mirror image of `bind`. How does that impact the code body and the type signature?
...
function bind<A, B>(mon: Mon<A>, f: (a: A) => Mon<B>): Mon<B> {
  switch (mon.kind) {
    case "value":
      return f(mon.value);
    case "exception":
...
function raise<A>(msg: ???): ??? {
  // TODO: Change JS code to TS
  return new Exn(msg);
}

function trywith<A>(mon: ???, func: (msg: ???) => ???): ??? {
  // TODO: Change JS code to TS
  if (mon instanceof Exn) {
    return func(mon.message);
  } else if (mon instanceof Value) {
...
function trywith<A>(mon: Mon<A>, func: (msg: string) => Mon<A>): Mon<A> {
  switch (mon.kind) {
    case "value":
      return mon;
    case "exception":
      return func(mon.exception);
  }
}

///// TESTS
run(raise<number>("Foo")) ==> "undefined"

///// KEYWORDS
ret: value
bind: exception, value
raise: exception
tryWith: exception, value
