...
You should not mutate the value of any pre-existing accumulator; you can only create new accumulator instances. Furthermore, you should not mutate the array. Furthermore, you cannot use for or while loops; use recursion only.
...
function foldFunctional(array, func, init) {
  ...
  if (array.length === 0) {
    ...
    let newAccum = func(last, init);
    return mapFunctional(rest, func, init);
  }
}

// TESTS
foldFunctional(adder, 0, [1, 2, 3]); ==> 6
foldFunctional(adder, 0, [-1, -2, -3]); ==> -6
foldFunctional(adder, -5, [0, 2, 4]); ==> 1
foldFunctional(adder, "", ["Foo", "Bar", "Qux"]); ==> QuxBarFoo
foldFunctional(subtract, 0, [2, 5, 7]); ==> 4
```
...
// TESTS
let arr = [1, 2]; findFunctional(greaterThan2, arr); ==> undefined
let arr = [1, 2, 3]; findFunctional(greaterThan2, arr); ==> 3
let arr = [3, 6, 10]; findFunctional(greaterThan2, arr); ==> 10
let arr = [-1, 3, 0, 5]; findFunctional(greaterThan2, arr); ==> 5
let arr = ["Foo", "Bar"]; findFunctional(x => { return x === "Foo"; }, arr); ==> "Foo" 
``` 
...
function filterFunctional(fn, list) {
  ...
  if (array.length === 0) {
    ...
    if (pred(last)) {
      return [...findFunctional(rest, pred), last];
    } else {
      return findFunctional(rest, pred);
    }
  }
...
// TESTS
let arr = [1, 2]; filterFunctional(greaterThan2, arr); ==> []
